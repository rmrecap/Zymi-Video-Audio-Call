...
router.post('/email/verify', requireAuth, (req, res) => {
  ...
  const result = verifyEmailOTP(req.user.id, otp);
  if (result.success) {
...
router.get('/phone/verify/:token', (req, res) => {
...
router.post('/phone/verify', (req, res) => {
  ...
  const result = verifyPhoneOTP(token, otp);
  if (result.success) {
    const status = checkTokenStatus(token);
    updateProfileCompletion(status.user_id);
    res.json({ success: true });
  } else {
    res.status(400).json({ error: result.error });
  }
